...
class AdminCalendarEventDetailView(APIView):
    ...
    def get(self, request):
        ...
        # 出欠記録を取得（当日分）
        # 注: LessonScheduleとの紐付けが必要な場合は別途対応
        attendance_dict = {}
        # ここでは簡略化のためにStudentEnrollment単位で出欠を管理
        ...
        for enrollment in enrollments:
            ...
            status_value = attendance_status.get('status', 'unknown')

            if status_value == 'present':
                present_count += 1
            elif status_value in ['absent', 'absent_notice']:
                absent_count += 1
