"""
チケットマスタデータインポートスクリプト (T7 - Ch形式)
T8_契約とチケット情報.xlsxの③T7_チケット情報シートからインポート
使用方法: docker compose run --rm backend python scripts/import_tickets_ch.py <xlsx_path>
"""
import sys
import os
import django
...
def import_tickets_ch(xlsx_path):
    """Chプレフィックスのチケットをインポート"""
    ...
    for row_num, row in enumerate(ws.iter_rows(min_row=2), start=2):
        try:
            ...
            if not ticket_code or not ticket_code.startswith('Ch'):
                continue
            ...
            # ブランドコード
            brand_code = str(row[4].value).strip() if row[4].value else ''
            brand = brand_map.get(brand_code)

            # チケット名
            ticket_name = str(row[6].value).strip() if row[6].value else ticket_code
            ...
            # Max値
            max_per_lesson = parse_int(row[7].value) or 1

            # 振替曜日
            transfer_day = parse_int(row[8].value)

            # 年間/週、チケット枚数
            annual_weekly = parse_int(row[9].value) or 42
            total_tickets = parse_int(row[10].value) or 42

            # 振替Group
            transfer_group = str(row[11].value).strip() if row[11].value else ''

            # 消化記号
            consumption_symbol = str(row[12].value).strip() if row[12].value else ''

            # 年マタギ利用
            year_carryover_raw = str(row[13].value).strip() if row[13].value else ''
            year_carryover = year_carryover_raw.upper() == 'OK'

            # カレンダーフラグ
            calendar_flag = parse_int(row[15].value) if len(row) > 15 else 2
            if not calendar_flag:
                calendar_flag = 2
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("使用方法: python scripts/import_tickets_ch.py <xlsx_path>")
        sys.exit(1)
    ...
    imported, updated, errors = import_tickets_ch(xlsx_path)
