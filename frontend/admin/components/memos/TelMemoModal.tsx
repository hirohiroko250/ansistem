...
export function TelMemoModal({ isOpen, onClose }: TelMemoModalProps) {
  ...
  const [callDirection, setCallDirection] = useState<string>("incoming");
  ...
  const handleSelectStudent = (student: Student) => {
    ...
    // 電話番号を自動設定
    const guardianPhone = student.guardian?.phone_mobile || student.guardian?.phone || "";
    setPhoneNumber(guardianPhone);
    setStep("form");
  };
  ...
  const handleSaveMemo = async () => {
    ...
    setIsSaving(true);
    try {
      await apiClient.post("/communications/tel-memos/", {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg font-bold text-green-600 bg-green-50 px-4 py-2 -mx-6 -mt-6 flex items-center gap-2">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <Select value={callDirection} onValueChange={setCallDirection}>
                    ...
                    <SelectContent>
                      <SelectItem value="incoming">着信</SelectItem>
                      <SelectItem value="outgoing">発信</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  ...
                  <Select value={callResult} onValueChange={setCallResult}>
                    ...
                    <SelectContent>
                      <SelectItem value="connected">通話</SelectItem>
